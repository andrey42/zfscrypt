...
extern int pam_sm_chauthtok(pam_handle_t* handle, int flags, int argc, char const** argv) {
    ...
    if (flags & PAM_UPDATE_AUTHTOK) {
        ...
        // When run via tests old_token is always NULL, this is temporary workaround.
        if (err.value == PAM_AUTHTOK_ERR && old_token == NULL && context.debug)
            err = zfscrypt_err_pam(0, "OK");
